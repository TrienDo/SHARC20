...
function viewAllMediaItems(selectedPOI)
{
    $('#dialog-message').html('');        
    //Title shows name of the entity and the number of media
    var count = selectedPOI.media.length;
    $('#dialog-message').dialog({ title: selectedPOI.poiDesigner.name + " (" + count + " media and " + selectedPOI.responses.length + " response(s))"});
    //Show all media items in an unordered list
    var content = getMediaContentWithResponses(selectedPOI.media) + getResponseContent(selectedPOI.responses);
    $('#dialog-message').append(content);
    $( "#dialog-message" ).dialog({
        modal: true,
        width: 370,
...
function showResponseAsPOI(rid){
    $('#dialog-message').html('');        
    //Title shows name of the entity and the number of media    
    $('#dialog-message').dialog({ title: "This response was submitted for a new location"});
    //Show all media items in an unordered list
    var tmpResponse = new Array();
    tmpResponse.push(allResponses[rid]);
    var content = getResponseContent(tmpResponse);
    $('#dialog-message').append(content);
    $( "#dialog-message" ).dialog({
        modal: true,
        width: 420,
...
function getMediaContentWithResponses(mediaExperience)//For displaying media pane
{
...
function getResponseContent(responses)//For displaying media pane
{
    var content = '<ul style="list-style:none; padding-left:0;display:table; margin:0 auto;">';
    for(var m = 0; m < responses.length; m++)
    {
    	var res = responses[m];
    	var responseText = "<div style='background-color:#66CCFF;'><p style='text-align:center;font-weight:bold;'> Your response submitted on " + res.submittedDate + "</p>";
    	//responseText += "<p style='text-align:left;margin-left:10'>" + res.desc + "</p>";    	
    	if(res.contentType == "text")
    		responseText += '<li id="' + res.id + '"><div><object class="textMediaBox" id="mediaPOI" type="text/html" data="' + res.content + '" ></object></div>';
        else if(res.contentType == "image")
        	responseText += '<li id="' + res.id + '"><img class="imgMedia" width="318" src="' + res.content + '"/>' + '<div class="formLabel">' + res.description + '</div>';        
