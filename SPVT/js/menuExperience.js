...
function askToSwitchAccount()
{
    $('#dialog-confirm').html('');        
    $('#dialog-confirm').dialog({ title: "Sharc Post Visit Tool"});           
    $('#dialog-confirm').append('<p>You have not consumed any experiences from this account! You may have used another account. Would you like to switch account?</p>');
...
function renderPOIs(retPOIs)    
{
    ...
    for(i = 0; i < retPOIs.length; i++) {
        ...
        var frameIcon = null;
		if(curPOI.responses.length > 0)
			frameIcon = new google.maps.MarkerImage("images/frameRes.png", null, null, new google.maps.Point(18,14), new google.maps.Size(36, 28));
		else
			frameIcon = new google.maps.MarkerImage("images/frame.png", null, null, new google.maps.Point(18,14), new google.maps.Size(36, 28));
										
		tmpFrameMarker = new google.maps.Marker({  
					   position: tmpLatLng, map: map, zIndex:2,visible: true,draggable: false,
					   icon: frameIcon, title: curPOI.name,id: allPOIMarkers.length	
					});
		addMarkerPOIClickEvent(tmpFrameMarker);                                        
		allPOIFrames.push(tmpFrameMarker);
...
function renderResponse(retResponses){
    for(i = 0; i < retResponses.length; i++) {        //( ,  ,  , , , , , , , , )
        ...
        //Viz marker
        var tmpLatLng = retResponses[i].entityId.split(" ");
        tmpLatLng = new google.maps.LatLng(parseFloat(tmpLatLng[0]), parseFloat(tmpLatLng[1]));        
        var frameIcon = new google.maps.MarkerImage("images/frameRes.png", null, null, new google.maps.Point(18,14), new google.maps.Size(36, 28));
        
		var tmpFrameMarker = new google.maps.Marker({  
					   position: tmpLatLng, map: map, zIndex:2,visible: true,draggable: false,
					   icon: frameIcon, title: curPOI.name,id: allPOIMarkers.length	
					});
		addMarkerPOIClickEvent(tmpFrameMarker);                                        
        ...
        var poiIcon = "images/poi.png";
        
        if(retResponses[i].contentType == "image")
            poiIcon = getImageMarker(retResponses[i].content);                        
        tmpPoiMarker = new google.maps.Marker({  
        			   position: tmpLatLng, map: map, zIndex:2,visible: true,draggable: false,
        			   icon: poiIcon, title: curPOI.poiDesigner.name, rid: i	
        		    });
        //markerManager.addMarker(tmpPoiMarker);
        addMarkerPOIClickEvent(tmpPoiMarker);                                
        allPOIMarkers.push(tmpPoiMarker);                 
